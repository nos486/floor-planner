export class Toolbar {
    ...
    this.container.innerHTML = `
      <div class="toolbar-pill">
        ...
        <div class="tool-group">
          ...
          <button class="tool-btn icon-only" id="btn-zoom-fit" title="Fit to Screen">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
  ...
  bindEvents() {
    ...
    this.container.querySelector('#btn-zoom-in').addEventListener('click', () => {
      this.state.ui.zoom = Math.min(5.0, this.state.ui.zoom * 1.25);
      this.canvas.applyTransform();
    });

    this.container.querySelector('#btn-zoom-out').addEventListener('click', () => {
      this.state.ui.zoom = Math.max(0.15, this.state.ui.zoom * 0.8);
      this.canvas.applyTransform();
    });
